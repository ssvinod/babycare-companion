...
export const useBabyStore = create<BabyStore>((set) => ({
    ...
    loadBaby: async () => {
        try {
            ...
            console.error('Unable to load baby:', error);
            set({
                baby: null,
                loading: false,
            });
    ...
    setBaby: async (baby) => {
        ...
        if (hasValidWeight && hasValidHeight) {
            ...
            const latest = await growthRepository.latest();
            const weightChanged = Number(latest?.weight ?? -1) !== Number(baby.weight);
            const heightChanged = Number(latest?.height ?? -1) !== Number(baby.height);
            if (!latest || weightChanged || heightChanged) {
                await growthRepository.insert({
                    ...
                    notes: latest ? 'Profile Updated' : 'Birth Measurement',
                });
            }
        }
    ...
    deleteBabyProfile: async () => {
        ...
        try {
            ...
            console.warn('Unable to cancel notifications:', error);
        }
        const repository = new DatabaseRepository();
        await repository.resetEverything();
        if (existingPhoto) {
            deleteProfilePhoto(existingPhoto);
        }
