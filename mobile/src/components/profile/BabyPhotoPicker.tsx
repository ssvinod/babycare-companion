...
export default function BabyPhotoPicker({ photo, gender, onChange }: Props) {
    ...
    async function handleResult(result: ImagePicker.ImagePickerResult) {
        ...
        try {
            setProcessing(true);
            const savedUri = saveProfilePhoto(result.assets[0].uri);
            onChange(savedUri);
        } catch (error) {
            console.error('Unable to save profile photo:', error);
            Alert.alert('Unable to save photo', 'The selected photo could not be saved.');
        } finally {
            setProcessing(false);
        }
    }
    ...
    return (
        <View style={styles.container}>
            ...
            >
                {photo ? (
                    ...
                    <View style={styles.fallback}>
                        <Text style={styles.fallbackText}>{fallbackAvatar}</Text>
                    </View>
                )}
            ...
            <Pressable disabled={processing} onPress={showOptions}>
                <Text style={styles.actionText}>
                    {photo ? 'Change Photo' : 'Add Photo'}
                </Text>
            </Pressable>
            <Text style={styles.helper}>Stored privately on this device</Text>
        </View>
    );
}
