...
function medicationSummary(pending: number, completed: number, skipped: number): string {
    const total = pending + completed + skipped;
    if (total === 0) {
        return 'No doses scheduled';
    }
    if (pending > 0) {
        return `${pending} pending • ` + `${completed} given`;
    }
    if (skipped > 0) {
        return `${completed} given • ` + `${skipped} skipped`;
    }
...
export default function DashboardScreen() {
    ...
    return (
        <ScreenLayout>
            ...
            {loading ? (
                <Text style={styles.statusText}>Refreshing today’s summary...</Text>
            ) : null}
            {!loading && error ? (
