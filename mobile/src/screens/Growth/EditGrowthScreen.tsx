...
type EditGrowthScreenProps = NativeStackScreenProps<ParamListBase, 'EditGrowth'>;
export default function EditGrowthScreen({ navigation, route }: EditGrowthScreenProps) {
    const { growth } = route.params as {
        growth: Growth;
    };
    const updateGrowth = useGrowthStore((state) => state.updateGrowth);
    const refreshDashboard = useDashboardStore((state) => state.refresh);
    const [checkupDate, setCheckupDate] = useState(growth.date.slice(0, 10));
    const [weight, setWeight] = useState(String(growth.weight));
    const [height, setHeight] = useState(String(growth.height));
    const [head, setHead] = useState(
        growth.headCircumference ? String(growth.headCircumference) : ''
    );
    ...
    async function save() {
        ...
        try {
            ...
            Alert.alert('Growth Updated', 'The growth record was updated successfully.', [
                {
                    text: 'OK',
    ...
    return (
        <ScreenLayout>
            <ScreenTitle title="Edit Growth" icon="📈" />
            ...
            />
            <PrimaryButton
                title={saving ? 'Updating...' : 'Update Growth'}
                onPress={() => {
                    if (!saving) {
                        void save();
                    }
