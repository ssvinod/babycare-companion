...
const doseRepository =
  new MedicationDoseRepository();
...
export default class MedicationDoseService {
  static async ensureDosesForDate(
  ...
  static async getTodayDoses(): Promise<
    TodayMedicationDose[]
  > {
    return this.getDosesForDate(
      getLocalDateString()
    );
  }
  static async markTaken(
    doseId: number
  ): Promise<void> {
    await doseRepository.markTaken(
      doseId
    );
  }
  static async markSkipped(
    ...
    await doseRepository.markSkipped(
      doseId
    );
  }
  static async markPending(
    ...
    await doseRepository.markPending(
      doseId
    );
  }
}