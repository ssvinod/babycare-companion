...
function medicationDoseTimestamp(
  dose: MedicationDoseTimelineRow
): string {
  if (dose.takenAt) {
    return dose.takenAt;
  }
...
export default class TimelineService {
  ...
  > {
    ...
    const feedingItems: TimelineItem[] =
      feedings.map(
        (
          feeding,
          index
    ...
    const sleepItems: TimelineItem[] =
      sleeps.map(
        (
          ...
          status: sleep.endTime
            ? "completed"
            : "active",
        })
      );
    const growthItems: TimelineItem[] =
      growthRecords.map(
        (
          growth,
          index
