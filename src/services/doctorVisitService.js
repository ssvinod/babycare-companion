...
export function addDoctorVisit(data) {
    const visit = data instanceof DoctorVisit ? data : new DoctorVisit(data);
...
export function getLatestDoctorVisit(childId) {
    const visits = repo.findByChild(childId);
...
export function getUpcomingDoctorVisit(childId) {
    ...
    return (
        repo
            .findByChild(childId)
            .filter(
                (visit) => visit.nextVisitDate && new Date(visit.nextVisitDate) >= today
            )
