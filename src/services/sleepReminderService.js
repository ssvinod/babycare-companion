...
function wakeWindow(ageMonths) {
    return (WAKE_WINDOWS.find((w) => ageMonths <= w.maxMonths) ?? WAKE_WINDOWS.at(-1))
        .minutes;
}
export function nextSleepTime(child, lastSleep) {
    if (!lastSleep) return null;
    const end = new Date(lastSleep.endTime);
    const wake = wakeWindow(child.ageMonths ?? 6);
    return new Date(end.getTime() + wake * 60000);
}
export function shouldSleepNow(child) {
    const last = getLatestSleep(child.id);
    if (!last) return true;
    return new Date() >= nextSleepTime(child, last);
}
