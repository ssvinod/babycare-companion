...
export function generateICS(child, vaccinations) {
    ...
    for (const visit of vaccinations) {
        ...
        const uid = (
            ...
            .toLowerCase()
            .replace(/\s+/g, '-');
        calendar += `UID:${child.name.toLowerCase()}-${uid}@babycarecompanion\r\n`;
        calendar += `DTSTAMP:${now}\r\n`;
        const eventDate = visit.dueDate ?? visit.date;
        calendar += `DTSTART;VALUE=DATE:${formatDateOnly(new Date(eventDate))}\r\n`;
        const title = visit.title ?? visit.visit ?? 'BabyCare Event';
        const descriptionLabel = visit.descriptionLabel ?? visit.type ?? 'Details';
        const items = [
            ...(visit.vaccines ?? []),
            ...(visit.milestones ?? []),
            ...(visit.details ?? []),
