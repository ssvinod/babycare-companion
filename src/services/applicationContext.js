...
export function createApplicationContext() {
    ...
    const growth = generateGrowthPlan(child);
    ...
    const parentInsights = generateParentInsights({
    ...
    const insights = parentInsights.messages;
    const insightSummary = parentInsights.summary;
    ...
    const timelineSummary = buildTimelineExportSummary(timeline);
    const babyStatus = buildBabyStatus({
        dashboard,
        healthScore,
    });
    const notifications = buildNotificationCenter({
        timeline,
        insights,
        recommendations,
    });
