...
export function addSleep(data) {
    const record = data instanceof SleepRecord ? data : new SleepRecord(data);
...
export function getTotalSleepToday(childId) {
    ...
    return getSleepRecords(childId)
        .filter((sleep) => new Date(sleep.startTime).toISOString().startsWith(today))
        .reduce((sum, sleep) => sum + sleep.durationMinutes, 0);
}
