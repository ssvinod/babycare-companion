export function generateParentInsights(context) {
    ...
    const formulaFeeds = feedings.filter((f) => f.type === 'Formula').length;
    const breastFeeds = feedings.filter((f) => f.type === 'Breastfeeding').length;
    const averageFeedVolume =
        formulaFeeds === 0
            ...
            : Math.round(
                  feedings
                      .filter((f) => f.quantity)
                      .reduce((sum, f) => sum + f.quantity, 0) / formulaFeeds
              );
    /* ---------------- Sleep ---------------- */
    const totalSleepMinutes = sleep.reduce((sum, s) => sum + (s.durationMinutes ?? 0), 0);
    const averageNap =
        sleep.length === 0 ? 0 : Math.round(totalSleepMinutes / sleep.length);
    const longestSleep = sleep.reduce(
        (max, s) => Math.max(max, s.durationMinutes ?? 0),
        0
    );
    /* ---------------- Medication ---------------- */
    const activeMedication = medications.filter((m) => !m.completed).length;
    const completedMedication = medications.filter((m) => m.completed).length;
    /* ---------------- Vaccination ---------------- */
    ...
    const completionPercent =
        vaccinations.length === 0
            ? 100
            : Math.round((completedVaccines * 100) / vaccinations.length);
    /* ---------------- Growth ---------------- */
    const latestGrowth = growth.length === 0 ? null : growth[growth.length - 1];
    /* ---------------- Health Score ---------------- */
    let score = 100;
    ...
    if (score < 75) level = 'Average';
    if (score < 50) level = 'Needs Attention';
    /* ---------------- Notifications ---------------- */
    const messages = [];
