...
export function generateDashboard(context) {
    const nextVaccination = context.vaccinations.find((v) => !v.completed);
    return {
        profile: {
            ...context.profile,
        ...
        vaccinations: {
            completed: context.vaccinations.filter((v) => v.completed).length,
            pending: context.vaccinations.filter((v) => !v.completed).length,
            next: nextVaccination
                ? {
                      ...nextVaccination,
                      dueDate: shortDate(nextVaccination.dueDate),
                      completedDate: nextVaccination.completedDate
                          ? shortDate(nextVaccination.completedDate)
                          : null,
                  }
                : null,
        },
        appointments: (context.appointments ?? []).map((v) => ({
            ...v,
            dueDate: shortDate(v.dueDate),
        })),
        reminders: context.reminders ?? [],
        milestones: (context.milestones ?? []).map((v) => ({
            ...v,
            dueDate: shortDate(v.dueDate),
        })),
        alerts: context.alerts ?? [],
        timeline: (context.timeline ?? []).map((v) => ({
            ...v,
            dueDate: shortDate(v.dueDate),
            completedDate: v.completedDate ? shortDate(v.completedDate) : null,
        })),
        sleep: context.sleep ?? {},
        feeding: context.feedingSummary ?? {},
        notifications: (context.notifications ?? []).map((n) => ({
            ...n,
...
export function getDashboard(child = loadProfile()) {
    ...
    const appointments = generateHealthVisitPlan(child);
    const milestones = generateMilestonePlan(child);
    const timeline = generateTimeline(child);
    const feedingSummary = getDailyNutritionSummary(child.id);
    const sleep = generateSleepInsights(child.id);
