...
export function generateTimeline(child) {
    ...
    const healthVisits = generateHealthVisitPlan(child);
    const visits = healthVisits.map(
        (visit) =>
            new Event({
                id: visit.id,
                type: 'health-visit',
    ...
    const appointments = generateAppointments(healthVisits).map(
        (appointment) =>
            new Event({
                id: appointment.id,
                type: 'appointment',
                ...
                details: [appointment.doctor, appointment.hospital],
            })
    );
    const feedings = getFeedings(child.id).map(
        (feed) =>
            new Event({
                id: feed.id,
                type: 'feeding',
                title: `${feed.type} Feeding`,
                dueDate: new Date(feed.date),
                details: [
                    feed.quantity ? `${feed.quantity} ${feed.unit}` : feed.duration,
                    feed.notes,
                ],
            })
    );
    const sleepEvents = getSleepRecords(child.id).map(
        (sleep) =>
            new Event({
                id: sleep.id,
                type: 'sleep',
                title: `${sleep.type} Sleep`,
                dueDate: sleep.startTime,
                details: [`${sleep.durationMinutes} minutes`],
            })
    );
    return [
        ...vaccinations,
