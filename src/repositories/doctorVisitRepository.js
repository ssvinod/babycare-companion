...
export class DoctorVisitRepository {
    ...
    load() {
        return db.load(COLLECTION).map((item) => new DoctorVisit(item));
    }
    findById(id) {
        return this.load().find((visit) => visit.id === id) ?? null;
    }
    findByChild(childId) {
        return this.load().filter((visit) => visit.childId === childId);
    }
    remove(id) {
