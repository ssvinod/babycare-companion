...
const currentFile = path.basename(import.meta.url.replace('file://', ''));

const filteredTests = tests.filter((file) => file !== currentFile);
...
for (const test of filteredTests) {
    process.stdout.write(`${test.padEnd(32, '.')}`);

    try {
        execSync(`node scripts/${test}`, { stdio: 'pipe' });
